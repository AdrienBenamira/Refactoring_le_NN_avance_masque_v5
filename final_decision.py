...
args.logs_tensorboard = args.logs_tensorboard.replace("test", "decision_TT")
...
df_final = pd.read_csv("./results/classification_all.csv")
...
ctdata0l, ctdata0r, ctdata1l, ctdata1r = nn_model_ref.c0l_train_nn, nn_model_ref.c0r_train_nn, nn_model_ref.c1l_train_nn, nn_model_ref.c1r_train_nn
liste_inputs = creator_data_binary.convert_data_inputs(args, ctdata0l, ctdata0r, ctdata1l, ctdata1r)
liste_inputs_v2 = []

for index_el in range(len(liste_inputs[0])):
    final_int = (int(liste_inputs[0][index_el])<<32) + (int(liste_inputs[2][index_el])<<16) + int(liste_inputs[3][index_el])
    liste_inputs_v2.append(final_int)
...
def create_mask(expici_clean):
    mask1 = [0]*48
    mask2 = [0]*48
    #res2 = 0
    #res1 = 0
    #cpt = 0
    for expici_clean_ici in expici_clean:
        expici_clean_ici_uniaue = expici_clean_ici.replace("~", "")
        if "DL" in expici_clean_ici_uniaue:
            offset = 0
            #offset2 = 32
            int_ici = int(expici_clean_ici_uniaue.replace("DL", "").replace("[", "").replace("]", ""))
        elif "V0" in expici_clean_ici_uniaue:
            offset = 16
            #offset2 = 16
            int_ici = int(expici_clean_ici_uniaue.replace("V0", "").replace("[", "").replace("]", ""))
        else:
            offset = 32
            #offset2 = 0
            int_ici = int(expici_clean_ici_uniaue.replace("V1", "").replace("[", "").replace("]", ""))
        #int_ici2 = 16 - int_ici
        #res2 += 2**(int_ici2 + offset2)
        #res2 += 140737488355328 >> (int_ici + offset)
        mask2[int_ici + offset] = 1
        if expici_clean_ici_uniaue == expici_clean_ici:
            mask1[int_ici + offset] = 1
            #res1 += 140737488355328 >> (int_ici + offset)
            #res1 += 2 ** (int_ici2 + offset2)
    #print(expici_clean)
    #print(mask1, bin(res1))
    #print(mask2, bin(res2))
    #print(cpt, np.sum(mask2))
    #res1m = int("".join(str(x) for x in mask1), 2)
    #res2m = int("".join(str(x) for x in mask2), 2)
    #assert res1==res1m
    #assert res2 == res2m
    #assert cpt == np.sum(mask2)

    return np.array(mask1), np.array(mask2)



def recursif_flag(exp, sample_bin):
    if len(exp)==0:
        return 1
    else:
        expici = exp[0]
        expici_clean = expici.replace("(", "").replace(")", "").split("|")
        M1, M2 = create_mask(expici_clean)
        nbre_de_uniaue_element_dans_mask_1 = np.sum(M2)
        rep = np.sum((sample_bin&M2)^M1)< nbre_de_uniaue_element_dans_mask_1
        if rep:
            return recursif_flag(exp[1:], sample_bin)
        else:
            return 0

all_conditions_1 = df_final['Expression'].values.tolist()
for index_s, sample in enumerate(liste_inputs_v2):
    cpteur = 0
    liste_ok = []
    string_s = format(sample, '048b')
    sample_bin = np.array([int(x) for x in string_s])
    for index_exp in range(len(all_conditions_1)):
        #print(index_exp)
        exp = all_conditions_1[index_exp].split("&")
        exp = sorted(exp, key=len)
        rep = recursif_flag(exp,sample_bin)
        if rep ==1:
            cpteur+=1
            liste_ok.append(index_exp)
    print(index_s, cpteur, Y_train_proba[index_s], liste_ok)
    if index_s == 100:
        print(ok)
